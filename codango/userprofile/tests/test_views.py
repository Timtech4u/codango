...
class UserProfileTest(StaticLiveServerTestCase):
    def setUp(self):
        ...
        self.notification = Notification.objects.create(
            id=100, content="Python", user=self.user, read=False, link="link",
            activity_type="Vote")
    ...
    def test_user_authenticate_with_github(self, m):
        m.get('https://api.github.com/user',
              content='{"login":"golden0","id":7931839}')
        m.post('https://github.com/login/oauth/access_token',
               content='{"access_token":"bcddf737641265ccf43ac82fea82d29a858e87be",\
               "token_type":"bearer","scope":"public_repo,user"}')
        m.get('https://api.github.com/users/golden0/repos',
              content='[{"language": "javascript"}]')
        response = self.client.get(
            reverse('user_github')+'?code=ieawfeaoefaojfeaoiw')
